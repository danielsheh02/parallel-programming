...
void selection_sort_partition_par(vector<double> &arr) {
    ...
    for (unsigned int i = 0; i < num_threads; i++) {
        ...
        if (i == num_threads - 1) {
            thread_data[i].end_idx = n - 1;
        } else {
            thread_data[i].end_idx = (i + 1) * chunk_size - 1;
        }
        
        if (pthread_create(&threads[i], nullptr, sort_chunk_thread, &thread_data[i]) != 0) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        size_t r = (i == num_threads - 1) ? (n - 1) : (l + chunk_size - 1);
